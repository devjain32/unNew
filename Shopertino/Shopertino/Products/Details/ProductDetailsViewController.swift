...
class ProductDetailsViewController: UIViewController {
    ...
    @objc private func didTapAddToBagButton() {
        let generator = UIImpactFeedbackGenerator(style: .light)
        generator.impactOccurred()
        cartManager.addProduct(product: product, quantity: 1, selectedColor: selectedColor, selectedSize: selectedSize)
...
extension ProductDetailsViewController: PKPaymentAuthorizationViewControllerDelegate {
    ...
    func paymentAuthorizationViewController(_ controller: PKPaymentAuthorizationViewController, didAuthorizePayment payment: PKPayment, completion: @escaping (PKPaymentAuthorizationStatus) -> Void) {
        if let placeOrderManager = placeOrderManager, let user = user {
            ...
            cart.addProduct(product: product, quantity: 1, selectedColor: selectedColor, selectedSize: selectedSize)
