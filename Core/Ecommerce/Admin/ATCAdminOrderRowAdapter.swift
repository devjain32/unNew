...
//                vc.use(adapter: ShoppingCartItemRowAdapter(uiConfig: uiConfig, hostViewController: uiEcommerceConfig), for: "ATCShoppingCartItem")
                vc.use(adapter: ShoppingCartItemRowAdapter(uiConfig: uiConfig, hostViewController: vc), for: "ATCShoppingCartItem")
                VCs[order.id] = vc
            }
            let vc = VCs[order.id]!
...
extension ATCAdminOrderRowAdapter: CPKSelectBottomControlDelegate {
    func didPickOption(_ option: CPKSelectOptionModel) {
        selectedCell?.statusButton.setTitle(option.title.uppercased(), for: .normal)
